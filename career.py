...
DB_COLUMNS = ["Event ID", "Title", "Organizer", "Venue", "Date", "Time", "Materials_Link", "Status"]
...
def send_admin_email(details):
    try:
        ...
        <html>
            <body>
                <h2>📌 New Career Event Request</h2>
                <ul>
                    <li><b>Organization:</b> {details['Org']}</li>
                    <li><b>Contact Email:</b> {details['Email']}</li>
    ...
    if target_password and admin_password == target_password:
        ...
        if not master_data.empty:
            ...
            for idx, row in master_data.iterrows():
                ...
                event_options.append((label, idx))

            selected_event_label = st.selectbox("Select Event to Cancel", [opt[0] for opt in event_options])
            cancel_reason = st.text_area("Reason for Cancellation")

            if st.button("Delete Event", type="primary"):
                selected_idx = [opt[1] for opt in event_options if opt[0] == selected_event_label][0]
                updated_df = master_data.drop(selected_idx).reindex(columns=DB_COLUMNS)
                conn.update(data=updated_df)
                st.cache_data.clear()
                st.success(f"Event cancelled successfully. Reason: {cancel_reason}")
                st.rerun()
        else:
            st.info("No active events found to cancel.")
    else:
...
# ==========================================
# TAB 1: CALENDAR FRAME & EVENT DETAILS
# ==========================================
with tab1:
    st.subheader("📅 Career Events Calendar")
    ...
    st.markdown(f"### 🔍 Event Details for **{selected_date_str}**")
    ...
    if not month_events.empty:
        ...
        if not day_details.empty:
            for _, row in day_details.iterrows():
                with st.expander(f"📌 [{row['Event ID']}] {row['Title']} ({row['Time']})", expanded=True):
                    st.write(f"**Event ID:** `{row['Event ID']}`")
                    st.write(f"**Venue:** {row['Venue']}")
                    st.write(f"**Organizer:** {row['Organizer']}")
                    st.write(f"**Status:** {row['Status']}")
        else:
            st.info("No career events scheduled for this date.")
    else:
        st.info("No events scheduled for this month.")

# ==========================================
# TAB 2: SEARCH BY EVENT ID & DOCUMENT PREVIEW
# ==========================================
with tab2:
    st.subheader("🔍 Extract Documents by Event ID")
    ...
    if search_id and not master_data.empty:
        # Match exact or partial Event ID
            ...
            for _, row in matched_event.iterrows():
                ...
                if doc_links:
                    ...
                    with col_prev:
                        ...
                        # Format Google Drive preview iframe if applicable
                    ...
                    st.warning(f"No document links attached to Event ID `{row['Event ID']}`.")
        else:
            st.error(f"No event found with Event ID matching '{search_id}'.")
    elif search_id:
        st.info("Database is empty.")

# ==========================================
# TAB 3: UPLOAD & CS-YY-MM-## EVENT ID GENERATION
# ==========================================
with tab3:
    st.subheader("📤 Upload Upcoming Career Event")
    st.warning("⚠️ Rule: Venue must be booked before the event is officially published.")

    with st.form("upload_event_form"):
        col1, col2 = st.columns(2)
        with col1:
            title = st.text_input("Career Event Title (e.g., UBD Talk)")
            organizer = st.text_input("Organizer / Department")
        ...
        with col2:
            venue = st.selectbox("Book Venue", [
                "Career Guidance Room", 
                "Lecture Theater Hall", 
                "Multi-Purpose Hall", 
                "Library Conference Room"
            ])
            materials_link = st.text_input("Document / Google Drive Links (Separate multiple links with commas)")

        submit_upload = st.form_submit_button("Book Venue & Publish Event")

    if submit_upload:
        if title and organizer and venue:
            ...
            if not clash.empty:
                ...
                # 1. Extract YY and MM from date
                yy = event_date.strftime("%y")
                mm = event_date.strftime("%m")

                # 2. Count existing events on that same exact date to compute ##
                ...
                st.cache_data.clear()
                st.balloons()
                st.success(f"✅ Venue '{venue}' booked! Generated Event ID: **{generated_event_id}**")
        else:
            st.error("Please fill in all required fields.")

# ==========================================
# TAB 4: EXTERNAL REQUESTS
# ==========================================
with tab4:
    st.subheader("✉️ Request to Hold Event from External Organization")
    ...
    with st.form("external_request_form"):
        ...
        event_proposal = st.text_area("Event Purpose & Proposal Summary")
        notify_method = st.selectbox("Send Notification Via", ["WhatsApp Link", "Automated Email", "Both"])

        submit_req = st.form_submit_button("Submit Event Request")

    if submit_req:
        if org_name and contact_email and event_proposal:
